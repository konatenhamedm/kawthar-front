<script lang="ts">
	...
	function init(form: HTMLFormElement) {
		((item.id = data?.id),
			...
			(item.emplacement_id = data?.emplacement_id),
			(item.nature_id = data?.nature_id),
			(item.etat_id = data?.etat_id),
			(item.date_acquisition = data?.date_acquisition),
			(item.date_mise_en_service = data?.date_mise_en_service),
			(item.cout_acquisition = data?.cout_acquisition),
			(item.image_url = data?.image_url),
			(item.date_scan = data?.date_scan),
			(item.observation = data?.observation),
			(item.personne_detentrice = data?.personne_detentrice));
	}
				...
				<div class="p-3">
					...
					{#if item.qr_bar_code || item.image_url}
						<div class="mb-6 flex flex-wrap items-start justify-between gap-4">
							...
							{#if item.image_url}
								<div class="flex flex-col items-center">
									<div
										class="cursor-pointer rounded-lg border bg-white p-2 shadow-sm"
										on:click={() => openLargeImage(item.image_url)}
									>
										<img
					...
					<div class="mb-3 grid grid-cols-4 gap-3">
						...
						<InputSimple
							...
							fieldName="etat_id"
							label="Etat"
							bind:field={item.etat_id}
							placeholder="Entrez etat"
						/>
						<InputSimple
							type="text"
							fieldName="emplacement_id"
							label="Emplacement"
							bind:field={item.emplacement_id}
							placeholder="Entrez emplacement"
						/>
					</div>
					<div class="mb-3 grid grid-cols-3 gap-3">
						<InputSimple
							...
							fieldName="nature_id"
							label="Nature"
							bind:field={item.nature_id}
							placeholder="Entrez nature"
						/>
						<InputSimple
							type="text"
					...
					<div class="mb-3 grid grid-cols-3 gap-3">
						...
						<InputSimple
							...
							label="Personne détentrice"
							bind:field={item.personne_detentrice}
							placeholder="Entrez une personne détentrice"
						/>
						<!-- <InputSimple
													type="text"
													fieldName="image_url"
...
{#if showLargeImage}
	...
	>
		<div class="max-h-screen max-w-4xl p-4" on:click|stopPropagation>
			<button
				class="absolute top-4 right-4 z-10 rounded-full bg-white p-2 shadow-md hover:bg-gray-200"
				on:click={closeLargeImage}
			>
				<svg
