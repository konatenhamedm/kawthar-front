<script lang="ts">
    ...
    async function confirmDelete() {
        isLoad = true;
        try {
            const res = await apiFetch(true , '/emplacements/delete/' + data?.id,'DELETE');
...
<div class="bg-white rounded-lg shadow p-6 space-y-4">
    ...
    <div class="text-center">
        ...
        <p class="text-gray-500">Êtes-vous sûr de vouloir supprimer cet etat ({data?.libelle}) ? Cette action est irréversible.</p>
    </div>
